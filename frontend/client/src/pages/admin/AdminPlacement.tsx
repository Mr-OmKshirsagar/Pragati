...
export default function AdminPlacement() {
  ...
  const evaluateRosterMutation = (trpc as any).placement?.evaluateRoster?.useMutation() ?? {
    mutateAsync: async () => {},
  };
  ...
  const handleRunRosterEvaluation = async (driveId?: string) => {
    ...
    if (!targetDriveId) {
      toast.error("Benchmark recruitment drive is not initialized yet.");
      return;
    }

    try {
      setIsEvaluatingRoster(true);
      toast.info("Running deterministic AST evaluation across candidate database...");
      const result = await evaluateRosterMutation.mutateAsync({ driveId: targetDriveId });
      setRosterModalData(result);
      toast.success(
        `Roster evaluated: ${result.eligibleCount} of ${result.totalEvaluated} candidates eligible!`
      );
    } catch (error: any) {
      toast.error(error.message || "Failed to evaluate candidate roster");
    } finally {
      setIsEvaluatingRoster(false);
    }
  };
  ...
  return (
    <AdminLayout currentPage="/admin/placement">
      <div className="p-4 sm:p-6 lg:p-8 space-y-6 max-w-7xl mx-auto">
        ...
        <div className="relative overflow-hidden rounded-3xl bg-gradient-to-r from-[#07172B] via-[#0C2D48] to-[#143D66] p-6 sm:p-8 text-white shadow-sm">
          ...
          <div className="relative z-10 flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
            ...
            <div className="flex flex-wrap items-center gap-3 shrink-0">
              <button
                onClick={() => handleRunRosterEvaluation()}
                disabled={isEvaluatingRoster}
                className="flex items-center gap-2 rounded-xl border border-white/20 bg-blue-600/80 px-4 py-2.5 text-xs sm:text-sm font-semibold text-white shadow-xs backdrop-blur-xs hover:bg-blue-600 active:scale-95 transition-all disabled:opacity-60"
              >
                {isEvaluatingRoster ? (
                ...
                <span>{isEvaluatingRoster ? "Evaluating AST..." : "Evaluate Roster (AST)"}</span>
              </button>
        ...
        {/* Roster Evaluation Results Modal */}
        {rosterModalData && (
...
function RosterEvaluationModal({
  data,
  onClose,
}: {
  data: any;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-xs p-4 animate-in fade-in duration-150">
      <div className="w-full max-w-4xl rounded-3xl bg-white shadow-2xl overflow-hidden border border-slate-200 flex flex-col max-h-[90vh]">
        ...
        <div className="flex items-center justify-between border-b border-slate-100 px-6 py-5 bg-gradient-to-r from-slate-900 to-indigo-950 text-white">
          <div className="flex items-center gap-3">
            <div className="grid h-10 w-10 place-items-center rounded-2xl bg-white/10 border border-white/20 text-blue-300">
              <Sparkles className="h-5 w-5" />
            </div>
            <div>
              <h2 className="text-base sm:text-lg font-extrabold tracking-tight">
                Deterministic AST Roster Evaluation
              </h2>
        ...
        <div className="p-6 bg-slate-50 border-b border-slate-200/70 grid grid-cols-2 sm:grid-cols-4 gap-3">
          ...
          <div className="p-3.5 rounded-2xl bg-emerald-50 border border-emerald-200/80 shadow-2xs">
          ...
          <div className="p-3.5 rounded-2xl bg-rose-50 border border-rose-200/80 shadow-2xs">
            ...
            <div className="text-xl sm:text-2xl font-extrabold text-rose-700 mt-1">
              {data.ineligibleCount}
            </div>
          </div>
          <div className="p-3.5 rounded-2xl bg-blue-50 border border-blue-200/80 shadow-2xs">
            <div className="text-[10px] font-bold uppercase tracking-wider text-blue-600">
              Pass Ratio
            </div>
            <div className="text-xl sm:text-2xl font-extrabold text-blue-700 mt-1">
              {data.totalEvaluated > 0
                ? Math.round((data.eligibleCount / data.totalEvaluated) * 100)
                : 0}
              %
            </div>
          </div>
        ...
        <div className="p-6 overflow-y-auto space-y-4 flex-1">
          <div className="flex items-center justify-between">
            <h3 className="text-xs font-bold uppercase tracking-wider text-slate-500">
              Candidate Decisions ({data.results?.length ?? 0})
            </h3>
            <span className="text-[11px] text-slate-400 font-medium">
              Zero-mock live database audit snapshot
            </span>
          </div>
              ...
              >
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                  <div className="flex items-center gap-3">
                    ...
                    >
                      {candidate.studentName?.charAt(0) || "S"}
                    </div>
                    <div>
                    ...
                    {candidate.reasons.map((reason: string, idx: number) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <span
                            className={
                              isPass ? "font-medium" : "font-semibold text-rose-700"
                            }
                          >
                            {reason}
                          </span>
        ...
        <div className="border-t border-slate-200 px-6 py-4 bg-slate-50 flex items-center justify-between">
          <div className="text-xs text-slate-500">
            Audit logs idempotently persisted in <code className="font-mono">eligibility_evaluations</code>
          </div>
          <button
            onClick={onClose}
            className="rounded-xl bg-slate-800 px-5 py-2.5 text-xs font-bold text-white hover:bg-slate-900 transition"
          >
            Close Roster View
          </button>
        </div>
