            ...
            <div className="flex h-[70px] items-center justify-between gap-4 px-4 sm:px-7 xl:px-10">
              <div className="flex min-w-0 items-center gap-3">
                <button
                  ...
                  className="rounded-lg p-2 text-[#52617d] hover:bg-white lg:hidden"
                >
                  <Menu className="h-5 w-5" />
                </button>
                <div className="hidden items-center gap-2 text-[11px] font-semibold text-[#71809a] sm:flex">
                ...
                <div className="truncate text-sm font-bold text-[#182643] sm:hidden">
                  PRAGATI / {title}
                </div>
              </div>
              <div className="flex items-center gap-2 sm:gap-3">
                <div className="hidden h-10 w-[230px] items-center gap-2 rounded-xl border border-[#dfe5ef] bg-white px-3 text-[#8994a8] shadow-sm md:flex">
                  <span className="text-xs">Search your workspace</span>
                  <span className="ml-auto rounded border border-[#e3e8f1] px-1.5 py-0.5 font-mono text-[10px]">
                    ⌘ K
                  </span>
                </div>
                <button
                  aria-label="Help"
                  className="hidden rounded-lg p-2 text-[#687691] hover:bg-white sm:block"
                >
                  <CircleHelp className="h-[18px] w-[18px]" />
                </button>
                <button
                  aria-label="Notifications"
                  className="relative rounded-lg p-2 text-[#687691] hover:bg-white"
                >
                ...
                <div className="flex items-center gap-2">
                  <span className="grid h-8 w-8 place-items-center rounded-xl bg-[#3048a8] text-[11px] font-bold text-white">
                    {initials}
                  </span>
                  <span className="hidden text-xs font-semibold text-[#304063] md:block">
...
function MobileNav({ items, activePath, onClose }: { items: NavItem[]; activePath: string; onClose: () => void }) { return <div className="fixed inset-0 z-50 lg:hidden"><button aria-label="Close navigation" onClick={onClose} className="absolute inset-0 bg-[#07112d]/50 backdrop-blur-sm" /><aside className="relative flex h-full w-[276px] flex-col bg-[#172446] text-white shadow-2xl"><div className="flex items-center justify-between"><Brand /><button aria-label="Close menu" onClick={onClose} className="mr-4 rounded-lg p-2 text-[#aab6d4] hover:bg-white/10"><X className="h-5 w-5" /></button></div><div className="px-3"><NavGroup label="Workspace" items={items} activePath={activePath} onNavigate={onCloseAndNavigate(onClose)} /><NavGroup label="Support" items={support} activePath={activePath} onNavigate={onCloseAndNavigate(onClose)} /></div></aside></div>; }
