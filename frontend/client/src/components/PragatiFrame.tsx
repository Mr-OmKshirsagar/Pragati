          ...
          <header className="sticky top-0 z-30 border-b border-[#e2e8f2]/90 bg-[#f5f7fb]/90 backdrop-blur-xl">
            <div className="grid h-[70px] grid-cols-[1fr_auto] items-center gap-4 px-4 sm:px-7 xl:px-10">
              <div className="grid grid-flow-col auto-cols-max items-center gap-3">
                <button aria-label="Open navigation" onClick={() => setMobileOpen(true)} className="rounded-lg p-2 text-[#52617d] hover:bg-white lg:hidden">
                ...
                <div className="truncate text-sm font-bold text-[#182643] sm:hidden">PRAGATI / {title}</div>
              </div>
              <div className="grid grid-flow-col auto-cols-max items-center gap-2 sm:gap-3">
                <div className="hidden h-10 w-[230px] grid-cols-[auto_1fr_auto] items-center gap-2 rounded-xl border border-[#dfe5ef] bg-white px-3 text-[#8994a8] shadow-sm md:grid">
                  ...
                  <span className="rounded border border-[#e3e8f1] px-1.5 py-0.5 font-mono text-[10px]">⌘ K</span>
                </div>
...
function NavGroup({ label, items, activePath, onNavigate }: { label: string; items: NavItem[]; activePath: string; onNavigate: (item: NavItem) => void }) {
  return (
    <div className="mb-8">
      <div className="mb-3 px-3 pt-2 text-[10.5px] font-bold uppercase tracking-[0.09em] text-[#8b99bd]">{label}</div>
        ...
        {items.map(item => {
          ...
          return (
            <Link
              ...
              className={`group grid grid-cols-[auto_1fr_auto] items-center gap-3 rounded-xl px-3 py-2.5 text-xs transition ${
...
function HelpCard() {
  return (
    <div className="m-3 rounded-xl border border-white/10 bg-white/[0.055] p-3">
      ...
      <Link href="/mentoring" className="grid grid-cols-[1fr_auto] items-center rounded-lg bg-[#3048a8] px-3 py-2 text-xs font-semibold text-white transition hover:bg-[#3e59bf]">
